...
function AddTask({onAddTaskSubmit}) {
    ...
    return (
      <div className="space-y-4 p-6 bg-slate-200 rounded-md shadow flex flex-col">
        ...
        className="border border-slate-300 outline-slate-400 px-4 py-2 w-full p-2 rounded-md" value={title} 
        onChange={(event) => setTitle(event.target.value)} />
        <input type="text" placeholder="Digite a descrição da tarefa" 
        className="border border-slate-300 outline-slate-400 px-4 py-2 w-full p-2 rounded-md" value={description} 
